...
module.exports = {
  ...
  async register(ctx) {
    ...
    const { email, username, provider, token } = params;
    ...
    // checking email / token is valid or not
    const inviteUser = await strapi.db.query('api::invite.invite').findOne({
        where: { email, token }
      });
      
      if (!inviteUser) {
        throw new ApplicationError('Email or token is invalid');
      }

      // update invited user isActivate field
